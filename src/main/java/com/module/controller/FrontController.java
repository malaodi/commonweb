...
public class FrontController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("pc/bangdan")
    public String bangdan(Model model, HttpSession session) {
        String sql = "select * from article a, (select tid,avg(score) as val from scoreinfo  GROUP BY tid order by val desc ) as b where a.id=b.tid limit 10";
        List<Map> mapList = commonMapper.selectAction(sql);
        model.addAttribute("beanDataList", mapList);
        return "pc/bangdan";
    }
